...
> 2026-09-06 Codex確認: 項目1・4・8・13・14は現行実装で対応済み。項目4・8・13・14の
> 回帰テストを通常テストへ昇格し、項目1には証拠レベルの回帰テストを追加した。
> 以下の各記録は発見時点の履歴として残す。
...
### 19. 攻略本PDF原本(スキャン画像)を直接精読して追加実装(項目18の続き、うち1件は解決)

- 対象コミット: 項目18と同じブランチの後続コミット。背景: ユーザーがChatGPT/Codexが読んだのと同じ攻略本スキャンPDF原本(4分割・計115頁)を直接渡し、「データ表ページだけでなく事細かに読み込んで」と依頼。原本を全頁精読した。詳細は `docs/decisions/0077-strategy-guide-primary-scan-deep-read.md` と `docs/research/strategy-guide-fixture-crosscheck-2026-09-16.md`(追記部分)を参照。
- **項目18-2(宣伝popularity_gain)を解決**: 原本の広告データページとオールテクニックガイドページの2箇所で独立に airship+40/radio+60/tv+90 を確認(合計4重確認)。`PROMOTIONS` を更新済み。
- **新規発見・未対応の矛盾2件**(上書きせず記録のみ):
  - `bench`/`fountain` の `service_bonus`: 既存+3/+25 vs 攻略本+4/+30。
  - 店舗「大」サイズの建設費: 攻略本の店舗選択画面ページで24,000,000円と印字されているが、データ表ページとレイアウト実例6箇所は一貫して18,000,000円。**攻略本自身の内部矛盾**(7箇所 vs 1箇所)。`STORE_VARIANTS`には多数決で18,000,000円を採用。
  - `editable_floor`(店舗内寸法)は既存値(`small_top=(8,13)`, `large_top=(13,14)`)がいずれの一次データとも一致せず、未解決のまま。
- **安全に追加したデータ**(未確認フィールドの穴埋めのみ、既存値は無変更): `SalaryTableEntry`のフィールド名を`monthly_salary_yen`→`hourly_wage_yen`に訂正(数値は元々正しかった、ラベルのみ攻略本原本で「時給」と確認)。`STORE_VARIANTS`の空欄建設費5件を追加。`TownFacilityAnchor`に`footprint`フィールドを新設し、誘致施設12件を新規追加、既存6施設にfootprint/shopping_populationを追記。
- **未実装のまま記録した公式群**(要判断・Codex側での実装検討を推奨): 店舗のサービス値/セキュリティ値/清掃値の計算式、新規出店の土地代計算式、宣伝効果の減衰式、店舗評価(★)の増減表、イベント発生条件表(寄付/万引き/火災・強盗/業界誌掲載/コンビニコンテスト/アイドル、いずれも正確な発生条件つき)、ゲームオーバー条件(破産or100年経過)。すべて`docs/research/strategy-guide-fixture-crosscheck-2026-09-16.md`の項目7に転記済み。`store_runtime.py`/`checkout.py`/`promotion.py`等への組み込みは、既存ロジックとの整合性検証が必要な別作業のため見送った。
- P1として引き続き保留(存在と読み取り可能性は確認済み): 店員候補ロースター約30名、顧客アーキタイプの時間帯別スケジュール約20層、建物×客の欲しい品物マトリクス約40種、設備×商品カテゴリの完全対応マトリクス、商品カテゴリごとの1回補給量・季節フラグ。

### 20. 項目19で保留したP1データ5件を同日中に転記完了

- 対象コミット: 項目19と同じブランチの後続コミット。背景: ユーザーから「P1として保留したデータは同じPDF内にあるのだから、できるなら全部転記しろ」との明示的な指示があり、5件すべてを再読・転記した。詳細は `docs/decisions/0078-strategy-guide-deferred-p1-datasets.md` を参照。
- **`STAFF_CANDIDATES`**(新規、35名): 氏名付き店員候補ロースターを全件転記(書籍頁127-133)。丸山昭夫のセキュリティ初期値のみ判読困難のため`None`のまま。
- **`CustomerVisitProfile`**(models.py新設データクラス)+**`CUSTOMER_ARCHETYPES`**(21層)+**`CUSTOMER_VISIT_SCHEDULE`**(140行): 顧客アーキタイプの時間帯別来店スケジュールを全件転記(書籍頁134-143)。既存の粗い`CustomerArchetypeDefinition`(spending_power_profile等)とは別概念として新設データクラスに格納(強制的に既存スキーマへ押し込まなかった)。9列の行動チューニング値(ス/素/マ/集/買/価/距/サ/平/休)は解像度制約で個別フィールド化する精度への自信が持てず、`behavior_stats_raw`という生タプルとして保持(時刻/所要時間/来店方法/所持金/欲しい商品は個別フィールドとして高信頼度で転記)。
- **`TownBuildingProfile`**(models.py新設)+**`TOWN_BUILDINGS`**(59件): 既存`TownFacilityAnchor`(誘致可能施設のみ)とは別に、マップ上の一般建物の規模・建物価格・欲しい品物・昼夜区分を全件転記(書籍頁92-95)。ページ右側の「主なほしい品物(深夜から早朝)」表は建物名ではなく商品組み合わせキーの別表と判明し、建物への対応付けは行っていない(誤った紐付けになるため)。時間帯の境界(朝/昼/夕/夜/深夜/早朝)も`TIME_OF_DAY_BUCKETS_HOURS`として記録。
- **`FixtureDefinition.compatible_product_categories`**: ○×マトリクス(小さな記号表、誤読リスク高)ではなく、各設備データページ自身の「取扱商品」テキスト列(大きな文字、書籍頁110-119)から約30什器+copier_a/bに転記。取扱商品「なし」のレジ2種と休憩室は`None`のまま。
- **`ProductCategoryPricing.seasonal_demand`**: 冷たい飲料/アイスクリーム=夏季、温かい飲料/おでん/中華まん=冬期の5カテゴリを追加。同表の残り2つの数値列(1回補給量、見出し不明列)は列の意味に確信が持てず転記を見送った(要判断として記録)。
...
### 22. 攻略本スキャンの再精読(コンテナ再作成後の再アップロード分、`restock_quantity`追加+要判断1件の発見)

- 背景: セッションのコンテナが再作成され、以前読んだPDF原本が失われたため、ユーザーが同じ4ファイルを再アップロードし、「最大限活用しろ」と明示的に再精読を指示。項目18/19/20で「解像度的に読めない」として保留していた項目を、単一ページ単位で再レンダリングして再試行した。
- **判明した事実**: 攻略本は物理的に2冊構成(「新人店長実習マニュアル/攻略&データブック」＝ファイル1+2、書籍頁6-143の独自ノンブル、と「レイアウトデザインセレクション74」＝ファイル3+4、こちらも書籍頁1から独自に再スタートし、中に2冊目の「クイックリファレンス」＋「パーソナルデザインBOOK」＋本物の「攻略&データブック」を内包、オールテクニックガイドは頁66-79、データリストは頁84-95)。項目18/19/20で記録した「書籍頁68-79」「86-89」等の頁番号は、実際にはこの第2巻内の頁番号であり、当時の引用は巻を取り違えていた(データソース自体は同一、頁番号の記録ミス)。
- **実装した新規データ**: `ProductCategoryPricing.restock_quantity`(新フィールド、`models.py`+`baseline_data.py`)。項目20で「列の意味に確信が持てず未転記」としていたDATA LIST商品表(書籍頁85、第2巻)の「1回補給」列を、単独ページの再レンダリングで再読し、全26カテゴリ分を転記。根拠: 同じ表の既存確認済み列(定価/原価率/利益率)が26/26全件で既存コードと完全一致したため、行×列の対応がズレていないことを独立に確認できた。
- **再確認(コード変更なし)**: オールテクニックガイド公式ページ(頁68-79)を再読し、項目21で実装済みの8公式(サービス値/セキュリティ値/清掃値/評価表/1-6怒り確率/万引き/寄付+5/宣伝減衰/土地代)がすべて同一内容で印刷されていることを確認。実装済みロジックの独立検証として記録。
- **調査したが未実装(理由付き)**:
  - **設備×商品カテゴリ○×マトリクス**(データリスト頁86-89): 視認性は改善したが、26列の無ラベル記号グリッドから列対応を復元するのは、既存の情報源(各設備ページ自身の大きな文字の「取扱商品」テキストリスト、項目20で使用済み)より信頼度が下がるため、あえて転記しなかった。
  - **社員スキル成長上限**(データリスト頁90-91、35名×5スキルの「初期値-上限値」レンジ): 新規データだが、クロスチェックで秋本三四郎の`academic_background`が既存コード上`70`のところ、この表では`65`と読め、**行×列の対応関係が信用できないことが判明**。数字を書き換えず、上限レンジも追加しなかった。要再検証。
  - **顧客アーキタイプの10列行動チューニング値**(`behavior_stats_raw`): 視認性は改善したが、約140行×10列を個別フィールド化するのは、社員表で発覚したのと同種のズレリスクを7倍のスケールで負うことになるため、今回は見送り。
  - DATA LIST商品表の残り2列(列見出し不明): 引き続き未解決。
- 要判断: 社員成長上限テーブルの行×列ズレの原因特定(単一ページ精読での再検証が必要)、顧客10列データの個別フィールド化を別途本格的に行うかどうか。
- 詳細: `docs/decisions/0080-strategy-guide-second-pass-restock-quantity.md`。

### 23. 項目22で保留した社員成長上限を、より信頼できる情報源から転記完了

- 背景: ユーザーから「未実装のまま報告するな、まず取り込んでから報告しろ」と明示的な指摘。項目22で保留した2件を再調査した。
- **社員スキル成長上限(実装完了)**: 項目22でズレが発覚したのは書籍頁90-91の「小さい文字の集約表」だった。同じ数値が、より大きな文字で個別ラベル付けされた「店員データ」個別カード(書籍頁126-133、項目18で一度閲覧済みだが転記していなかった)にも掲載されていることを確認し、そちらから再転記した。**35名全員について、既に確認済みの初期値5項目(サービス/レジ/清掃/補充/セキュリティ)をこのカードから読み直し、既存コードと突き合わせたところ34/34(セキュリティ初期値が判明している全員)が完全一致**し、行×列対応の信頼性を確認できた。`StaffDefinition`に`*_skill_growth_ceiling`を5フィールド追加(`models.py`)、`STAFF_CANDIDATES`全35件に値を設定(`baseline_data.py`)。丸山昭夫はセキュリティ初期値は引き続き不明のままだが、セキュリティ成長上限(50)は別セルなので転記済み。
- **顧客10列データ(再挑戦したが今回も見送り)**: 同じ書籍頁134の凡例(⑦〜⑰の番号付き)で、ス/素/マ/集/買/価/距/サ/平/休の**定義文そのもの**は高信頼度で確認できた(`CustomerVisitProfile`のdocstringを更新)。ただし実データ行の桁数を再度数え直したところ、同一アーキタイプの隣接行で9個・11個など数え直すたびに不一致が生じ、社員表と異なり照合可能な既知の参照値が無いため、個別フィールド化はやはり見送った。「試していない」のではなく再現性の壁に実際に当たった結果である。
- 詳細: `docs/decisions/0085-staff-growth-ceilings-from-per-candidate-cards.md`。
