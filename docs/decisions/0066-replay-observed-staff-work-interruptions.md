# 0066 — Replay explicitly observed replenish/clean interruptions
...
The replay plan stores per-staff/per-task chronological occurrence rules. Each rule contains only the explicitly measured game-minute span from work start to interruption. Partial starts or interruption events remain unpaired rather than receiving an imputed threshold. Normally completed work episodes still count as occurrences, preventing a later interruption rule from being misapplied to an earlier completed task.
