...
### 追記(2026-09-24): プロジェクトオーナー本人の直接プレイ証言

プロジェクトオーナー本人から、初代PS版の直接プレイ経験に基づく証言を得た:

> 対象の商品棚を選択し、中身が減っていると補充のコマンドが出て、プレイヤーが
> 任意で補充できたはず

これはCLAUDE.mdのevidence tier(1)「direct-play observation」に該当する
CONFIRMED_COMMUNITY相当の一次証言であり、攻略本・wiki等の既存資料には
まだ独立に裏付けが無い(=このセクション本文の他の記述とは出典の性質が異なる)。
`manual_restock_action`自体はこれによりUNKNOWNから「存在する」に格上げできるが、
以下は依然として未確定のまま:

- 「減っている」の正確な閾値(何個以下で補充コマンドが出現するか)
- 発注ロット数・コスト計算式
- コマンドの正確な名称・UI配置

game/側の実装(`game/scripts/main.gd`の`_selected_fixture_restock_target()`、
docs/decisions/0149-*.md)では、上記の「対象棚を選択→在庫が閾値以下でのみ
補充コマンドが有効化される」という構造のみをこの証言から実装し、閾値の具体的な
値は決定書0089で導入済みの`restock_trigger_stock_units_at_or_below`
