...
func _initialize() -> void:
    ...
    if simulation.staff.checkout_staff().staff_id != str(config["staff"]["checkout_staff_id"]):
        ...
        return
    var initial_cash := simulation.economy.cash_yen
    var initial_stock := simulation.inventory.total_stock_units()
    ...
    var initial_fixture_snapshot := simulation.layout.fixture_snapshot()
    ...
    var initial_shelf_origin := simulation.layout.fixture_origin(shelf_id)
    ...
    var sales_after_sellout := simulation.economy.completed_sales
    if not simulation.start_next_customer():
    ...
    var first_record_before_mutation := simulation.economy.sale_records[0].duplicate(true)
    var first_customer_id := str(first_record_before_mutation["customer_id"])
    var first_customer: CustomerState = simulation.customers.customers[first_customer_id]
